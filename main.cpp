...
int main()
{
    ...
    srand(time(NULL)); //Untuk membuat karakter ditempatkan secara random di awal sebelum permainan di mulai

//Membuat pembatas (pagar) pada game
   for (i=0;i<=79;i++)
    {   gotoxy(i,0);
        cout<<"#";
        gotoxy(i,40);
...
//Pengacakan lokasi karakter (lakon)
    lakonx = rand()%40 + 20;
    lakony = rand()%20 + 10;
    gotoxy(lakonx,lakony); cout << '\2'; // '\2' pada ASCII table berupa gambar karakter smile
...
//Membuat pergerakan karakter
    while(true)
    {
        delay++; //Untuk memperlambat pergerakan prosesor
        ...
        if(delay>limit)
        {
            ...
            delay=0;
             ...
             if(musuhx==buahx&&musuhy==buahy)
             {
                ...
                gotoxy(buahx,buahy); cout << '@';
                skormusuh+=100;
                limit-=100; //Menambah kecepatan bergerak setelah mendapat target (buah)
       ...
       if(lakonx>78||lakonx<1||lakony>39||lakony<1) //Untuk membuat perbatasan gerak karakter
       {
       ...
       { //penekanan tombol
           ...
           {gotoxy(lakonx,lakony);cout << ' ';
                 switch(arah)
                {
                    //Membuat pergerakan jumping pada karakter
           ...
           if(ch<0)
           {
               ...
               {
                 //Memberikan proteksi gerak arah
